        ...
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-sm">
                        <div class="mb-4">
                            <a href="{{ route('products.create') }}" type="button" class="btn btn-primary waves-effect waves-light"><i class="bx bx-plus me-1"></i> Add Product</a>
                    ...
                    <div class="col-sm-auto text-end">
                        <a href="{{ route('products.index') }}" class="btn btn-warning mb-4">Reset Filter</a>
                    </div>
                </div>
                <!-- end row -->
                        ...
                        <tbody>
                            <form id="formBulkDelete" action="{{ route('products.destroy', 'id') }}" method="POST">
                                ...
                                <tr>
                                    ...
                                    <td>{{ $item->code }}</td>
                                    <td>{{ $item->name }}</td>
                                    <td>{{ enumText($item->type) }}</td>
                                    <td>{{ $item->product_category->name }}</td>
                                    <td>{{ formatCurrency($item->stock, '') }}</td>
                                    {{-- <td>{{ $item->description }}</td> --}}
                                    <td>
                                        <div class="d-flex flex-row gap-2">
                                            <a href="{{ route('products.show', $item->id) }}" class="btn btn-light"><i class="fas fa-eye"></i></a>
                                            <a href="{{ route('products.edit', $item->id) }}" class="btn btn-primary"><i class="fas fa-pencil-alt"></i></a>
                                            <form action="{{ route('products.destroy', $item->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger" data-confirm-delete="true"><i class="fas fa-trash"></i></button>
                                            </form>
                                        </div>
                                        {{-- <div class="row">
    ...
    <script>
        $(function(){
            $('.btn-edit').click(function(){
                ...
                // console.log(data.id);
                let form = $('#formEdit');
                let url = "{{ route('products.update', ':id') }}".replace(':id', data.id);
